...
func TestNewAddLinkEvent(t *testing.T) {
	tests := []struct {
		name                           string
		key                            *string
		igpRouterId                    *string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			event := NewAddLinkEvent(link)
			assert.NotNil(t, event)
		})
	}
}

func TestAddLinkEvent_GetKey(t *testing.T) {
	tests := []struct {
		name string
		link Link
		want string
